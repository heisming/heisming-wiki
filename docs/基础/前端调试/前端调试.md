...
```
![](assets/img/source_bundle_map.png)

可以关联，也可以不关联，比如加上 hidden，就是生成 sourcemap 但是不关联：
```js
module.exports = {
  entry: './src/index.js',
  devtool: 'hidden-source-map', // 特性
  mode: 'development',
  // ...
}
```
![](assets/img/hidden_source_bundle_map.png)
...
module.exports = {
  entry: './src/index.js',
  devtool: 'inline-source-map', // 特性
  mode: 'development',
  // ...
}
```

![](assets/img/inline_source_bundle_map.png)

### cheap
```js
module.exports = {
  entry: './src/index.js',
  devtool: 'cheap-source-map', // 特性
  mode: 'development',
  // ...
}
```
...
![](assets/img/cheap_source_map.png)

### module
```js
module.exports = {
  entry: './src/index.js',
  devtool: 'module-source-map', // 特性
  mode: 'development',
  // ...
}
```
...
![](assets/img/sourcemap_sources_content.png)
当你加上 nosources 之后，生成的 sourcemap 就没有 sourceContent 部分了：
```js
module.exports = {
  entry: './src/index.js',
  devtool: 'nosources-source-map', // 特性
  mode: 'development',
  // ...
}
```
...
- module = true : 表示loader是否生成source map。
- columns = true : 表示是否应该使用column mapping。
- noSources = false : 防止源文件的内容被包含在source map 中。
...
- `^(inline-|hidden-|eval-)?(nosources-)?(cheap-(module-)?)?source-map$`的规律来进行组合

当然，这种 sourcemap 配置还不够细致，比如 sourcemap 的 url 怎么生成，文件名是什么。如果想对这些做配置，可以关掉 devtool，启用 SourceMapDevToolPlugin 来配置。

> 虽然 webpack 的 sourcemap 配置方式比较多，但最底层也就是浏览器支持的文件级别的 sourcemap 还有 eval 代码的 source 映射和 sourcemap 这两种机制。其余的方式都是基于这两种机制的封装。
